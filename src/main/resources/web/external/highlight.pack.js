var hljs = new function() {
	...
	function a(q) {
		var p = (q.className + " " + (q.parentNode ? q.parentNode.className
				: "")).split(/\s+/);
		p = p.map(function(r) {
	...
	function d(E, G, C, M) {
		...
		function D(N, r) {
			...
			if (t(r, B)) {
				throw new Error('Illegal lexem "' + r + '" for mode "'
						+ (B.cN || "<unnamed>") + '"')
			}
			w += r;
			return r.length || 1
	...
	function m(r, u, p) {
		...
		var o = c(r);
		if (o.length) {
			var q = document.createElementNS("http://www.w3.org/1999/xhtml",
					"pre");
			q.innerHTML = w.value;
			w.value = j(o, c(q), v)
		}
	...
	function n() {
		...
		Array.prototype.map.call(
				document.getElementsByTagNameNS("http://www.w3.org/1999/xhtml",
						"pre"), b).filter(Boolean).forEach(function(o) {
			m(o, hljs.tabReplace)
		})
	}
	function k() {
