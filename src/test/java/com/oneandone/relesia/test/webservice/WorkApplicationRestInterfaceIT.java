...
public class WorkApplicationRestInterfaceIT {

	private final String RESOURCE_PATH = "workapps";
	private final String NEW_OBJECT_JSON = "{\"name\": \"WORKAPPFORTEST\","
			+ "\"application\": {\"id\": 1},"
			+ "\"localPath\": \"someLocalPath\","
			+ "\"tag\": \"somtesttag\","
	...
	public void requestNewObjectCreation_thenCreationIsSuccesfull() throws ClientProtocolException, IOException {
		...
		// Then
		WorkApplicationEntity resource = RestTestUtil.retrieveResourceFromResponse(response,
				WorkApplicationEntity.class);
	...
	public void requestObjectUpdate_thenUpdateIsSuccesfull() throws ClientProtocolException, IOException {
		...
		resource = RestTestUtil.retrieveResourceFromResponse(response, WorkApplicationEntity.class);
		Assert.assertNotNull(resource);
