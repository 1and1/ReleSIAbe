...
public class PluginRestInterfaceIT {

	private final String RESOURCE_PATH = "plugins";
	private final String NEW_OBJECT_JSON = "{\"name\": \"SOME_PLUGIN_FOR_TEST\","
			+ "\"type\": \"CHECKER\","
			+ "\"className\": \"com.oneandone.relesia.checker.FORTEST\","
			+ "\"properties\": ["
			+ "{"
			+ "\"name\": \"propFORTEST1\","
			+ "\"value\": \"valFORTEST1\""
	...
	public void requestNewObjectCreation_thenCreationIsSuccesfull() throws ClientProtocolException, IOException {
		...
		// Then
		PluginEntity resource = RestTestUtil.retrieveResourceFromResponse(response,
				PluginEntity.class);
	...
	public void requestObjectUpdate_thenUpdateIsSuccesfull() throws ClientProtocolException, IOException {
		...
		resource = RestTestUtil.retrieveResourceFromResponse(response, PluginEntity.class);
		Assert.assertNotNull(resource);
