...
public class BuildTypeRestInterfaceIT {

	private final String RESOURCE_PATH = "buildTypes";
	private final String NEW_OBJECT_JSON = "{\"toolName\": \"NEWBUILDTOOLFORTEST\"," + "\"version\": \"3.x\","
			+ "\"supportsDependency\": true," + "\"supportsBuild\": true," + "\"supportsStaging\": false,"
			+ "\"supportsRelease\": true," + "\"supportsAuth\": true}";
	...
	public void requestNewObjectCreation_thenCreationIsSuccesfull() throws ClientProtocolException, IOException {
		...
		// Then
		BuildTypeEntity resource = RestTestUtil.retrieveResourceFromResponse(response, BuildTypeEntity.class);
	...
	public void requestObjectUpdate_thenUpdateIsSuccesfull() throws ClientProtocolException, IOException {
		...
		Assert.assertNotNull(resource);
		String newName = resource.getToolName() + "_UPDATED_DURING_TEST";
		resource.setToolName(newName);
		...
		Assert.assertNotNull(resource);
		resource = RestTestUtil.retrieveResourceFromResponse(response, BuildTypeEntity.class);
		Assert.assertEquals(resource.getToolName(), newName);
	}
