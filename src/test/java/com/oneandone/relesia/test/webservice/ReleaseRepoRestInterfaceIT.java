...
public class ReleaseRepoRestInterfaceIT {

	private final String RESOURCE_PATH = "releaseRepos";
	private final String NEW_OBJECT_JSON = "{\"name\": \"NEWRELEASEREPOFORTEST\","
			+ "\"type\": \"maven\","
			+ "\"url\": \"some.url.for.test.com\","
			+ "\"supportsAuth\": true}";
	...
	public void requestNewObjectCreation_thenCreationIsSuccesfull() throws ClientProtocolException, IOException {
		...
		// Then
		ReleaseRepoEntity resource = RestTestUtil.retrieveResourceFromResponse(response, ReleaseRepoEntity.class);
	...
	public void requestObjectUpdate_thenUpdateIsSuccesfull() throws ClientProtocolException, IOException {
		...
		resource = RestTestUtil.retrieveResourceFromResponse(response, ReleaseRepoEntity.class);
		Assert.assertNotNull(resource);
