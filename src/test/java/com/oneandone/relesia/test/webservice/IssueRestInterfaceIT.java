...
public class IssueRestInterfaceIT {

	private final String RESOURCE_PATH = "issues";
	private final String NEW_OBJECT_JSON = "{\"application\": {\"id\": 1},"
			+ "\"subject\": \"ISSUEFORTEST\","
			+ "\"description\": \"something happened there\","
			+ "\"refferencePath\": \"an URL must be added here\","
			+ "\"issueType\": \"CHANGELOG\","
			+ "\"status\":\"NEW\"}";
	...
	public void requestNewObjectCreation_thenCreationIsSuccesfull() throws ClientProtocolException, IOException {
		...
		// Then
		IssueEntity resource = RestTestUtil.retrieveResourceFromResponse(response,
				IssueEntity.class);
	...
	public void requestObjectUpdate_thenUpdateIsSuccesfull() throws ClientProtocolException, IOException {
		...
		Assert.assertNotNull(resource);
		String newName = resource.getSubject() + "_UPDATED_DURING_TEST";
		resource.setSubject(newName);
		...
		resource = RestTestUtil.retrieveResourceFromResponse(response, IssueEntity.class);
		Assert.assertNotNull(resource);
		Assert.assertEquals(resource.getSubject(), newName);
	}
